...
#Random is the same so we leave it outside of for loop
np.savetxt("random.txt"%i,np.transpose([z_r,np.cos(th_r),phi_r,w_r]))


## FOR LOOP FOR DIFFERENT SAMPLES:
for i in xrange(31,100+1):
    #make a sample
    ngal = z_d.size
    indices = np.random.choice(ngal,ngal,replace=True)

    #Run CUTE
    np.savetxt("data_%s.txt"%i,np.transpose([z_d[indices],np.cos(th_d[indices]),phi_d[indices],w_d[indices]]))
    stout= "data_filename= data_%s.txt\n"%i
    stout+="random_filename= random.txt\n"
    stout+="input_format= 0\n"
    stout+="output_filename= corr_3D_ps_highres_nbin60_fkp_%s.txt\n"%i
    ...
    stout+="dim3_min= 0.\n"
    stout+="dim3_nbin= 20\n"
    f=open("param_cute_highres_nbin60_fkp_%s.ini"%i,"w")
    f.write(stout)
    f.close()
    os.system("../CUTEdir/CUTE/CUTE param_cute_highres_nbin60_fkp_%s.ini > log_cute_highres_nbin60_fkp_%s.txt"%(i,i))
    os.system("rm data_%s.txt"%i)
    os.system("cat log_cute_highres_nbin60_fkp_%s.txt"%i)
